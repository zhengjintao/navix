...
public class FarmDocmessageDao extends HibernateSQLTools<FarmDocmessage> implements FarmDocmessageDaoInter {
	...
	public int getAllListNum() {
		...
		SQLQuery sqlquery = session
				.createSQLQuery("select count(*) from navix_docmessage");
	...
	public int getNoReadMessageNum(String userId) {
		Session session = sessionFatory.getCurrentSession();
		SQLQuery sqlquery = session
				.createSQLQuery("select count(*) from navix_docmessage where READUSERID=? and READSTATE='0'");
	...
	public int getAppMessageNum(String appid) {
		Session session = sessionFatory.getCurrentSession();
		SQLQuery sqlquery = session
				.createSQLQuery("select count(*) from navix_docmessage where APPID=?");
