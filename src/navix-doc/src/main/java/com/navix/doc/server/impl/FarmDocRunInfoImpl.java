...
import com.navix.core.util.web.FarmproHotnum;
import com.navix.core.util.web.WebVisitBuff;
import com.navix.doc.dao.FarmDocDaoInter;
import com.navix.doc.dao.FarmDocenjoyDaoInter;
import com.navix.doc.dao.FarmDocmessageDaoInter;
...
import com.navix.doc.domain.FarmDocenjoy;
...
import com.navix.doc.util.HtmlUtils;
...
public class FarmDocRunInfoImpl implements FarmDocRunInfoInter {
	...
	public DataQuery createReleaseRankingSimpleQuery(DataQuery query) {
		DataQuery dbQuery = DataQuery.init(query,
				"(SELECT A.USERNAME AS USERNAME, A.DOCNUM AS DOCNUM, A.GOODNUM AS GOODNUM, A.BADNUM AS BADNUM,"
						...
						+ "	FROM (SELECT * FROM NAVIX_DOC A WHERE A.STATE = '1') DOC "
						+ "	INNER JOIN NAVIX_DOCRUNINFO DOCRUNINFO ON DOC.RUNINFOID = DOCRUNINFO.ID"
						+ "	LEFT JOIN navix_AUTH_USER USER ON DOC.CUSER = USER.ID" + "	GROUP BY DOC.CUSER) A )B",
	...
	public DataQuery getUserEnjoyDoc(String userId) {
		DataQuery query = DataQuery.getInstance("1",
				...
				"NAVIX_DOCENJOY a left join NAVIX_DOC b on a.DOCID=b.ID and b.STATE = '1' left join NAVIX_DOCRUNINFO c on c.id =b.RUNINFOID");
	...
	public List<DocBrief> getNewKnowList(int pagesize) {
		DataQuery query = DataQuery.getInstance("1",
				...
				"NAVIX_doc a LEFT JOIN NAVIX_docruninfo b ON a.RUNINFOID=b.ID LEFT JOIN NAVIX_rf_doctype c ON c.DOCID=a.ID LEFT JOIN NAVIX_doctype d ON d.ID=c.TYPEID   LEFT JOIN navix_AUTH_USER e ON e.ID=a.CUSER");
	...
	public List<DocBrief> getTypeDocs(String typeid, String userid, int num) {
		...
		query.addRule(new DBRule("d.TREECODE", doctype.getTreecode(), "like-"))
				...
				.addSqlRule("and (a.READPOP='1' or a.CUSER='" + userid
						+ "' OR (a.READPOP = '2' AND 0 < (SELECT e.ID FROM NAVIX_docgroup_user e WHERE e.PSTATE = 1 AND e.GROUPID = a.DOCGROUPID AND e.CUSER = '"
						+ userid + "')))");
		query.setPagesize(num);
		List<DocBrief> list = null;
		try {
	...
	public List<DocBrief> getPubTopDoc(int num) {
		...
		try {
			...
			query = DataQuery.init(query, "NAVIX_TOP TOP " + "LEFT JOIN NAVIX_DOC A ON TOP.DOCID = A.ID and a.STATE='1' "
					+ "LEFT JOIN NAVIX_RF_DOCTYPE B ON B.DOCID =A.ID " + "LEFT JOIN NAVIX_DOCTYPE C ON C.ID=B.TYPEID "
	...
	public DataResult getStatGoodUsers(int i) {
		try {
			...
			query.setPagesize(i);
			query = DataQuery.init(query,
					"( SELECT d.ID AS ID,d.NAME AS NAME,SUM(b.PRAISEYES) AS SUMYES FROM NAVIX_DOC a LEFT JOIN NAVIX_DOCRUNINFO b ON a.RUNINFOID = b.ID LEFT JOIN navix_AUTH_USER d ON a.CUSER = d.ID where a.STATE='1' GROUP BY d.ID,d.NAME ) g",
					"ID,NAME,SUMYES");
	...
	public DataResult getStatGoodGroups(int i) {
		try {
			...
			query.setPagesize(i);
			query = DataQuery.init(query,
					"( SELECT d.ID   AS ID, d.GROUPNAME AS NAME,d.JOINCHECK as JOINCHECK, SUM(b.PRAISEYES) AS SUMYES FROM NAVIX_DOC a LEFT JOIN NAVIX_DOCRUNINFO b ON a.RUNINFOID = b.ID LEFT JOIN NAVIX_DOCGROUP d ON d.id=a.DOCGROUPID WHERE a.STATE='1' and d.ID IS NOT NULL GROUP BY d.ID,d.GROUPNAME,d.JOINCHECK) g",
	...
	public DataResult getStatMostUsers(int i) {
		try {
			...
			query.setPagesize(i);
			query = DataQuery.init(query,
					"( SELECT b.ID AS ID, b.name AS NAME, COUNT(*) AS NUM FROM NAVIX_DOC a LEFT JOIN navix_AUTH_USER b ON a.CUSER = b.ID where a.STATE='1' GROUP BY b.id,b.NAME) g",
					"ID,NAME,NUM");
	...
	public DataResult getStatGoodDocs(int i) {
		try {
			...
			query.setPagesize(i);
			query = DataQuery.init(query,
					"NAVIX_DOC a LEFT JOIN NAVIX_DOCRUNINFO b ON a.RUNINFOID=b.ID LEFT JOIN NAVIX_DOCGROUP c ON a.DOCGROUPID=c.ID LEFT JOIN navix_AUTH_USER d ON a.CUSER=d.ID",
	...
	public Map<String, Object> getStatNumForDay() throws Exception {
		...
		DataQuery query = new DataQuery();
		query = DataQuery.init(query,
				"(SELECT COUNT(id) AS num,LEFT(ctime,8) AS DATE FROM NAVIX_doc GROUP BY DATE) AS a", "NUM,DATE");
		...
		DataQuery queryGood = DataQuery.init(query,
				"(SELECT COUNT(a.id) AS num,LEFT(a.ctime,8) AS DATE FROM NAVIX_doc a LEFT JOIN NAVIX_docruninfo b ON a.RUNINFOID=b.ID WHERE b.EVALUATE>0  GROUP BY DATE) AS a",
				"NUM,DATE");
	...
	public DataResult getStatUser(User user) {
		try {
			...
			sql.append(
					"SELECT a.NAME AS NAME,a.ID AS ID,a.ctime AS ctime,(SELECT COUNT(*) FROM NAVIX_DOC WHERE cuser=a.ID and STATE='1') num,(SELECT MIN(ctime) FROM NAVIX_DOC WHERE cuser=a.ID) stime,(SELECT MAX(ctime) FROM NAVIX_DOC WHERE cuser=a.ID) etime,");
			sql.append(
					"(SELECT SUM(pb.PRAISEYES) FROM NAVIX_DOC pa LEFT JOIN NAVIX_DOCRUNINFO pb ON pb.id=pa.RUNINFOID WHERE cuser=a.ID and pa.STATE='1') AS OKNUM,");
			sql.append(
					"(SELECT SUM(pb.PRAISENO) FROM NAVIX_DOC pa LEFT JOIN NAVIX_DOCRUNINFO pb ON pb.id=pa.RUNINFOID WHERE cuser=a.ID and pa.STATE='1') AS NONUM,");
			sql.append(
					"(SELECT SUM(pb.EVALUATE) FROM NAVIX_DOC pa LEFT JOIN NAVIX_DOCRUNINFO pb ON pb.id=pa.RUNINFOID WHERE cuser=a.ID and pa.STATE='1') AS EVANUM,");
			sql.append(
					"(SELECT SUM(pb.VISITNUM) FROM NAVIX_DOC pa LEFT JOIN NAVIX_DOCRUNINFO pb ON pb.id=pa.RUNINFOID WHERE cuser=a.ID and pa.STATE='1') AS VINUM,");
			sql.append(
					"(SELECT COUNT(*) FROM NAVIX_DOC pa LEFT JOIN NAVIX_DOCRUNINFO pb ON pb.id=pa.RUNINFOID WHERE cuser=a.ID AND pb.EVALUATE>0 and pa.STATE='1') AS GOODNUM,");
			sql.append(
					"(SELECT COUNT(*) FROM NAVIX_DOC pa LEFT JOIN NAVIX_DOCRUNINFO pb ON pb.id=pa.RUNINFOID WHERE cuser=a.ID AND pb.EVALUATE<0 and pa.STATE='1') AS BADNUM");
			sql.append(" FROM navix_AUTH_USER a WHERE a.ID='" + user.getId() + "'");
	...
	public DataResult getMyAuditingByUser(String userid, int pagesize, int pagenum) {
		DataQuery query = DataQuery.getInstance(pagenum,
				...
				"NAVIX_DOC_AUDIT b left join NAVIX_DOC a on a.id=b.DOCID LEFT JOIN  NAVIX_rf_doctype c ON c.DOCID=a.ID LEFT JOIN NAVIX_doctype d ON d.ID=c.TYPEID  left join NAVIX_DOCTEXT f on f.id=b.TEXTID");
