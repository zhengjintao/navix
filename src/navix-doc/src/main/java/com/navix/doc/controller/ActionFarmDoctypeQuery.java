	...
	public Map<String, Object> queryall(DataQuery query, HttpServletRequest request) {
		try {
			...
			DataResult result = farmDocManagerImpl.createSimpleTypeQuery(query).search()
					.runDictionary(FarmParameterService.getInstance().getDictionary("NAVIX_DOCTYE_TYPEMOD"), "TYPEMOD")
					.runDictionary(FarmParameterService.getInstance().getDictionary("NAVIX_DOCTYE_DOCMOD"), "CONTENTMOD")
					.runDictionary("1:结构,2:建设,3:商品,4:链接,5:单页", "TYPE").runDictionary("1:可用,0:禁用", "PSTATE")
	...
	public Map<String, Object> loadTreeNode(DataQuery query, HttpServletRequest request, String id) {
		...
		try {
			List<EasyUiTreeNode> treeNodes = EasyUiTreeNode.formatAsyncAjaxTree(
					EasyUiTreeNode.queryTreeNodeOne(id, "SORT", "NAVIX_DOCTYPE", "ID", "PARENTID", "NAME", "CTIME")
							.getResultList(),
					EasyUiTreeNode.queryTreeNodeTow(id, "SORT", "NAVIX_DOCTYPE", "ID", "PARENTID", "NAME", "CTIME")
							.getResultList(),
