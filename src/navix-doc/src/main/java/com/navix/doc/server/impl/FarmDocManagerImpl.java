...
import com.navix.doc.domain.FarmRfDoctype;
...
import com.navix.doc.server.commons.FarmDocFiles;
...
import com.navix.doc.util.DocTagUtils;
...
public class FarmDocManagerImpl implements FarmDocManagerInter {
	...
	public DocEntire editDoc(DocEntire paradoce, String editNote, LoginUser user) {
		...
		// 处理版本信息
		{
			// 审核相关代码(处理版本，分类未改变时商品新建立版本并设置为待审核状态)
			if (newType != null && !newType.getAuditpop().equals("0")) {
				...
				usermessageServiceImpl.sendMessage(farmDocRunInfoImpl.getDocJoinUserIds(newDoce.getDoc().getId()),
						"商品‘" + newDoce.getDoc().getTitle() + "’有新版本提交", "商品信息更新消息");
	...
	public DataQuery createSimpleDocQuery(DataQuery query) {
		DataQuery dbQuery = DataQuery.init(query,
				"NAVIX_DOC  A LEFT JOIN NAVIX_RF_DOCTYPE B ON B.DOCID =A.ID LEFT JOIN NAVIX_DOCTYPE C ON C.ID=B.TYPEID LEFT JOIN NAVIX_docgroup d ON d.ID=a.DOCGROUPID",
	...
	public DataQuery createSimpleTypeQuery(DataQuery query) {
		DataQuery dbQuery = DataQuery.init(query, "NAVIX_doctype a LEFT JOIN NAVIX_doctype b ON a.PARENTID=b.id",
	...
	public List<FarmDoctext> getDocVersions(String docId) {
		...
		DataQuery dbQuery = DataQuery.getInstance("1", "ID,ETIME,CUSERNAME,DOCID,PCONTENT,PSTATE", "NAVIX_doctext");
