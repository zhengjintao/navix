...
public class WebFileController extends WebUtils {
	...
	@Resource
	private NavixWebFileManagerInter navixWebFileManagerImpl;
	@Resource
	...
	public ModelAndView editCommit(String docid, String fileId, String knowtype, String knowtitle, String knowtag,
		...
		try {
			...
			doc = navixWebFileManagerImpl.editWebFile(docid, Arrays.asList(fileId.trim().split(",")), knowtype, knowtitle,
		...
		try {
			...
			if (creattype != null && creattype.equals("on")) {
				...
				for (String fileid : fileids) {
					...
					doc = navixWebFileManagerImpl.creatWebFile(fileidlist, knowtype, fileName, knowtag, docgroup, text,
				...
				doc = navixWebFileManagerImpl.creatWebFile(fileids, knowtype, knowtitle, knowtag, docgroup, text,
						POP_TYPE.getEnum(writetype), POP_TYPE.getEnum(readtype), getCurrentUser(session));
				if (doc.getAudit() != null) {
