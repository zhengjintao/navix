	...
	<div class="stream-list p-stream">
		<div class="stream-item" id="loop-30">
			...
			<a class="thumbnail"> <c:if test="${docgroup.groupimg==null}">
					<img src="UI-WEB/docgroup/commons/imgDemo.png" alt="店铺">
				</c:if> <c:if test="${docgroup.groupimg!=null}">
					<img src="${docgroup.imgurl}" alt="店铺">
				</c:if>
			</a>
...
<div class="widget-box shadow-box">
	<div class="title">
		<h3>
			<i class="glyphicon glyphicon-fire"></i> 关注用户
		</h3>
	</div>
	<div class="stream-list p-stream">
		<div class="stream-item" id="loop-30">
		...
		if (confirm('是否确定取消关注店铺？')) {
			...
			$.post('webgroup/haveAdministratorIs.html', {
				...
				if (flag.adminNum > 1) {
					...
					if (confirm('如果退出该店铺，该店铺将没有任何管理员。是否确定退出？')) {
						window.location = basePath
								+ 'webgroup/leaveGroup.do?groupId=' + id;
					}
				}
