package com.navix.parameter.dao.impl;
...
import com.navix.core.sql.result.DataResults;
...
public class ParameterDao implements ParameterDaoInter {
	...
	public int getAllListNum() {
		Session session = sessionFatory.getCurrentSession();
		SQLQuery sqlquery = session.createSQLQuery("select count(*) from navix_Parameter");
	...
	public List<Map<String, Object>> getListByDomainType(String domainType) {
		String sql = "select a.id, a.name, a.vtype, a.rules, a.pvalue, a.comments " + "from navix_PARAMETER a "//
				+ "where a.domain = ? " //
